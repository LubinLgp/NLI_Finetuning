...
Nous nous concentrons sur les erreurs du \textbf{finetuning}, qui donne les meilleurs résultats sur NLI4PR. L'analyse s'appuie sur les exemples où les trois modèles finetunés (NLI, Clinical Matching, CoT) se trompent~: 196 en POL et 174 en MEDICAL, avec une forte proportion de vrais labels \textbf{Contradiction} (environ 75--80\,\%). Les 123 \textbf{candidats} sont issus de l'intersection POL–MEDICAL de ces indices (même patient en erreur dans les deux formulations) et forment un noyau dur où la tâche reste difficile quelle que soit la formulation.

Une \textbf{analyse linguistique} a été menée sur ces 123 candidats, en les comparant au reste des indices du jeu de test (comme pour NLI4CT). Les métriques utilisées sont~: chevauchement lexical (Jaccard), couverture lexicale, densité numérique (chiffres, pourcentages, unités), nombre de négations, longueur de la prémisse et du statement, et présence de mots-clés d'essai. Les tableaux~\ref{tab:nli4pr-linguistic-pol} et~\ref{tab:nli4pr-linguistic-med} résument les moyennes candidats vs autres pour les indicateurs les plus discriminants (POL et MEDICAL). Les candidats se distinguent par des \textbf{prémisses plus courtes} (environ 198 mots vs 252 pour les autres), une \textbf{densité numérique plus faible} (environ 31--33 vs 37--40), et une \textbf{couverture lexicale légèrement plus basse}. En MEDICAL, les candidats ont en moyenne \textbf{plus de négations dans le statement} (1,72 vs 1,42). Ces profils suggèrent que le noyau dur d'erreurs correspond à des cas où les critères sont moins longs et moins chargés en données numériques, avec des formulations potentiellement plus ambiguës (négations). Les figures~\ref{fig:nli4pr-comparison-pol} et~\ref{fig:nli4pr-comparison-medical} illustrent la comparaison détaillée des métriques entre candidats et autres indices.
